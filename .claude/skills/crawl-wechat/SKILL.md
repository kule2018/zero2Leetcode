...
使用 Python 3.10 或更高版本运行脚本。确保已安装依赖；建议在虚拟环境中执行，`crawl4ai-setup` 会安装浏览器运行时：
...
脚本仅将 JSON 摘要输出到标准输出，进度和预览写入标准错误，并可选择将完整 HTML 和/或 Markdown 保存到文件。只接受 `https://mp.weixin.qq.com/s/...` 公开文章链接。
...
article = asyncio.run(crawl_wechat_article(
    ...
    image_url_prefix="images",     # Markdown/HTML 中使用 POSIX 相对路径
))
print(article["title"])
...
- 部分临时分享链接会在一段时间后过期
- 仅抓取有权访问和保存的公开内容；文章及图片版权不因本工具的 MIT 许可而改变
- 复杂 SVG（尤其数学公式）在 Markdown 转换中可能丢失，需保留 HTML 核对

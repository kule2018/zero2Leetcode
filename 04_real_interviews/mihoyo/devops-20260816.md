...
- 第 1 题不要直接枚举所有子数组。由 $|a_i|\le 100$ 可以推出合法子数组长度不超过 $100$，据此将枚举量降到 $O(100n)$。
...
- 第二行输入 $n$ 个整数 $a_1,a_2,\ldots,a_n$，满足 $|a_i|\le100$。
...
- 值域约束经常可以推导出枚举维度的常数上界。看到 $|a_i|\le100$ 和目标值 $L^2$ 时，应立即比较区间最大和 $100L$ 与目标值。

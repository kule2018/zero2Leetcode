...
> 原始公开文章中的部分数学公式和数据范围在抓取时未被保留。本文只整理能由题意、样例和原文代码确认的信息，不补写缺失的约束。原文：[百度秋招 7.30 算法岗笔试题解](https://mp.weixin.qq.com/s/0STtyWW3fcC_otVt4DnzZw)；延伸资料：[大厂笔试真题汇总](https://svih06ofn7v.feishu.cn/wiki/E4SBwR7ZKiSZxpkRVZCcmjI9nBl)、[校招投递信息汇总](https://docs.qq.com/smartsheet/DUXJLSnZoTVFhVUVs?tab=sc_K49vrh)。
...
输入包含多组测试。每组先给出括号串长度 $n$，再给出长度为 $n$、仅由 `(` 和 `)` 构成的合法括号串。原始抓取结果中的具体数据范围缺失，因此此处不补写。
...
给定长度为 $n$ 的整数序列 $a$ 和正整数模数 $M$。序列首尾相接，视为一个环。

从每个起点出发，依次取环上后续元素，最多取 $n$ 个。走出第 $t$ 步后，计算前 $t$ 个已取元素之和对 $M$ 的标准余数，并将这些余数组成轨迹。轨迹不包含尚未取元素时的初始余数。

只要下一步得到的余数与轨迹中已有余数重复，游走就停止；若一直没有重复，则取满 $n$ 步后停止。记从起点 $i$ 出发能取到的最大步数为 $L_i$，求

$$
\sum_{i=1}^{n} L_i.
$$

输入包含多组测试。每组先输入 $n$ 和 $M$，再输入 $n$ 个整数。原始抓取结果中的具体数据范围缺失，因此此处不补写。
...
$$
p_0=0,\qquad p_j=(p_{j-1}+a_{(j-1)\bmod n})\bmod M.
$$
...
用 $0$ 下标表示原数组。若从位置 $s$ 出发，走 $t$ 步后的轨迹值为

$$
(p_{s+t}-p_s)\bmod M.
$$

对于固定起点 $s$，每个轨迹值都减去了同一个常数 $p_s$。因此两个步数的轨迹值相等，当且仅当对应的前缀余数相等。

也就是说，从起点 $s$ 出发走 $L$ 步合法，等价于

$$
p_{s+1},p_{s+2},\ldots,p_{s+L}
$$

两两不同。原问题由“环上每个起点的余数轨迹”转化为“前缀余数数组中，每个左端点开始的最长无重复窗口”。
...
**第四步：为什么使用集合而不是按模数开数组**

原始抓取结果未保留 $M$ 的范围。若直接分配长度为 $M$ 的标记数组，在 $M$ 很大时可能浪费内存。哈希集合只保存当前窗口中至多 $n$ 个余数，不依赖缺失的数据范围。
...
在上述规则下，使最终序列的乘积尽可能大，并输出最大乘积对 $10^9+7$ 取模的结果。该模数可由原公开文章的代码确认；原始抓取结果中的其余具体数据范围未完整保留，因此不补写。

输入包含多组测试。每组先输入 $n$ 和 $k$，再输入 $n$ 个正整数。
...
持续递减当前最大值，会把所有冒尖元素逐渐压到同一水平附近。于是可以先选择整数水平线 $x$，把所有大于 $x$ 的元素削到 $x$。

达到水平线 $x$ 所需操作数为

$$
cost(x)=\sum_{i=1}^{n}\max(0,a_i-x).
$$

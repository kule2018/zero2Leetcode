...
输入为不定长列表，用 Pandas 自动对齐并填零，保证所有样本维度一致。
...
train_df = pd.DataFrame(data["train"]).fillna(0)
test_df = pd.DataFrame(data["test"]).fillna(0)
...
# 测试集对齐维度
if X_test.shape[1] < d:
    X_test = np.hstack([X_test, np.zeros((X_test.shape[0], d - X_test.shape[1]))])

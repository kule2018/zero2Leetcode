...
输入为不定长列表。**关键**：必须先从每行末尾提取标签，再对特征做零填充对齐。若直接用 `pd.DataFrame` 对齐整行再取最后一列，短行的标签会被填充值覆盖，导致标签错位。
...
# 先提取标签（每行最后一个元素），再对齐特征
train_raw = data["train"]
y = np.array([row[-1] for row in train_raw], dtype=float)
features = [row[:-1] for row in train_raw]

# 特征零填充对齐
d = max(len(f) for f in features)
X_train = np.zeros((len(features), d))

...
description: 为 Zero2Leetcode 的本地练习页批量补充题目描述、代码模板、测试用例与接线逻辑。适用于扫描缺失题目、按最多 5 个并行代理分批写入 `assets/js/playground-extra/`，并最终接入 `playground.html` 与 `assets/js/playground.js`。
---
...
- `.agents/cnblogs-publisher/published_links_*.json`
  若 `problems-data.js` 的 `blogUrl` 不全，可从这里补齐已发布博客链接。
